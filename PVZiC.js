...
packetX = undefined
 drawCost(cost) {
    this.ctx.fillText(cost, Math.round(this.packetX + seedPacket.width - 69), 72)
}
...
peashooterPeaSpawnRate = 82
 ...
 peashooterActions() {
    this.peashooters.forEach(p => {
        ...
        if (p["peaFrame"] >= this.peashooterPeaSpawnRate / 2 && p["animFrame"] >= 30) {
            this.zombies.filter(z => p["row"] == z["row"] && p["x"] <= z["x"]).forEach((z, zi) => {
                this.peas.push({
